...
class db_mysqli implements db_interface {
	...
	public function table_maxid($key) {
		...
		if($query) {
			...
			throw new Exception('framework_maxid error, mysqli_error:'.mysqli_error());
		}
		if($maxid === FALSE) {
	...
	public function table_count($table) {
		...
		if($query) {
			...
			throw new Exception('framework_cout error, mysqli_error:'.mysqli_error());
		}
		if($count === FALSE) {
	...
	public function query($sql, $link = NULL, $isthrow = TRUE) {
		...
		if(!$result && $isthrow) {
			$s = 'mysqli Query Error: <b>'.$sql.'</b>. '.mysqli_error();
	...
	public function mysqli_result($result, $number, $field=0) {
        mysqli_data_seek($result, $number);
        $row = mysqli_fetch_array($result);
        return $row[$field];
    }
	public function result($query, $row) {
		return mysqli_num_rows($query) ? intval($this->mysqli_result($query, $row)) : FALSE;
	}
	/**
	 * 获取 mysqli 版本
	 * @return string
