...
class core{
	...
	public static function ob_start() {
		...
		//callback $output_callback, int $chunk_size, bool $erase
		//两种写法
	...
	public static function init_set() {
		...
		spl_autoload_register('core::autoload_handler');	// 设置自动包含类文件方法
	...
	public static function autoload_handler($classname) {
		if(substr($classname, 0, 3) == 'db_') {
			include FRAME_PATH.'db/'.$classname.'.class.php';
		}elseif(substr($classname, 0, 6) == 'cache_') {
			include FRAME_PATH.'cache/'.$classname.'.class.php';
		}elseif(is_file(FRAME_PATH.'ext/'.$classname.'.class.php')) {
			include FRAME_PATH.'ext/'.$classname.'.class.php';
		}elseif(is_file(ROOT_PATH.'library/'.$classname.'.class.php')) {
			include ROOT_PATH.'library/'.$classname.'.class.php';
		}else{
	...
	public static function init_get() {
        //前台伪静态
		if(!empty($_ENV['_config']['parseurl'])) {
			...
			//强制设定项目名 默认为home
			$match = preg_match("/^(home|admin)/i",$u);
			if(!$match) $u = "home/".$u;
			$uArr = explode('/', $u);
			//截取项目
			if(isset($uArr[0])) {
		...
		//设置默认项目、控制器、方法
		$_GET['project'] = isset($_GET['project']) && preg_match('/^\w+$/', $_GET['project']) ? $_GET['project'] : C('projects.0');
	...
	public static function init_control() {
		//根据项目来定义变量
		defined('CONTROL_PATH') || define('CONTROL_PATH', ROOT_PATH.'control/'.$_GET['project'].'/');	//控制器目录
		defined('VIEW_PATH') || define('VIEW_PATH', TPL_PATH.$_GET['project'].'/');	//视图目录
		defined('RUNTIME_CONTROL') || define('RUNTIME_CONTROL', RUNTIME_PATH.$_GET['project'].'_control/');	
		//控制器缓存目录
		...
		if(empty($_ENV['_config']['plugin_disable'])) {
			...
			if(isset($plugins['enable']) && is_array($plugins['enable'])) {
				...
				foreach($plugin_enable as $p) {
					// 第1步 查找 plugin/xxx/HOM_NAME/xxx.(php|htm)
					if(is_file(PLUGIN_PATH.$p.'/'.HOM_NAME.'/'.$filename)) {
						return PLUGIN_PATH.$p.'/'.HOM_NAME.'/'.$filename;
					}
	...
	public static function clear_code($s) {
		...
		$s = ltrim($s);
		if(substr($s, 0, 29) == 'defined(\'FRAME_PATH\') || exit;') $s = substr($s, 29);
