...
class view{
	...
	public function display($filename = null) {
		...
		extract($this->vars, EXTR_SKIP);//提取键值对成为变量 如果有冲突，不覆盖已有的变量
		include $this->get_tplfile($_ENV['_tplname']);
	}

	//视图模版临时文件
	private function get_tplfile($filename) {
		$view_dir = $_GET['project'].($_ENV['_view_diy'] ? '_view_diy' : '_view').'/';
		...
		if(!is_file($php_file) || DEBUG) {
			...
			if(FW($php_file, $this->tpl_parse($tpl_file)) === false) {//解析模板文件同时写入runtime目录
	...
	private function tpl_parse($tpl_file) {
		...
		$s = preg_replace_callback('#\{inc\:([\w\.]+)\}#', 'self::parse_inc', $s);
		...
		//第8步 路径问题
		$website = $_ENV["_config"]["hostname"];
		$website = preg_replace("/(http:\/\/)?((\w+\.?)+)\/?/i", "http://$2/", $website);
		$url = $website."tpl/".$_GET["project"]."/".$_ENV["_theme"]."/";
		$s = preg_replace('#="((css|js|img)\/.+\.\w+)"#', '="'.$url.'\\1"', $s);
		...
		$s = "<?php defined('HOM_NAME') || exit('Access Denied'); $head_str\r\n?>$s";
		$s = str_replace('?><?php ', '', $s);
	...
	private function parse_block($matches) {
		...
		$lib_file = core::get_original_file('kp_block_'.$func.'.lib.php', VIEW_PATH.$_ENV['_theme'].'/block/');
