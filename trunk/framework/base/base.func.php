 ...
 * @return mixed
 * @example C('db.master.host') 将返回 string 'localhost'
 */
function C($key, $val = null) {
	...
	if(!preg_match('/\.+/', $key)){
		...
		$res = array();
		//是否有该配置项
		if(!isset($_ENV['_config'][$key[0]])){
			//是否创建
			if(is_null($val)) return $val;
			else $_ENV['_config'][$key[0]] = array();
		}
		$res = &$_ENV['_config'][$key[0]];
		$tmp = count($key);
		//循环遍历
		for($i = 1; $i < $tmp; $i++){
			//是否有该配置项
			if(!isset($res[$key[$i]])){
				//是否创建
				if(is_null($val)) return $val;
				else $res[$key[$i]] = ($i+1) != $tmp ? array() : $val;
			}
			$res = &$res[$key[$i]];
		}
		return $res;
