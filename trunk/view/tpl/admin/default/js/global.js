...
window.gfAjax = {
	...
	loading : function() {
		...
		$(window).resize(gfAjax.setTopLeft);
		gfAjax.setTopLeft();
	},
	...
	unObj : function() {
		$("object,select").each(function(){
			if($(this).css("visibility") != "hidden") $(this).attr("_gf_bugs_visibility_gf_", $(this).css("visibility")).css("visibility", "hidden");
		});
	},

	//显示object,select
	disObj : function() {
		$("[_gf_bugs_visibility_gf_]").each(function(){
			$(this).css("visibility", $(this).attr("_gf_bugs_visibility_gf_")).removeAttr("_gf_bugs_visibility_gf_");
		});
	},
	...
	remove : function() {
		...
		window.isIE6 && gfAjax.disObj();
		$(".ajaxoverlay,.ajaxtips").remove();
	},

	//关闭
	close : function() {
		$(".ajaxtips").animate({top:0}, 250, gfAjax.remove);
	},

	//设置提示框位置
	setTopLeft : function(H) {
		if($(".ajaxtips").length == 0) return;
		$(".ajaxtips").css({"top":gfAjax.getHeight(H), "left":gfAjax.getWidth()});
	},
	...
	//设置提示框动画
	setTopAn : function(H) {
		var T = gfAjax.getHeight(H);
		$(".ajaxtips").css({"top":0, "left":gfAjax.getWidth()}).animate({top:T+10}, 150).animate({top:T-20}, 150).animate({top:T}, 150);
	},

	//写入对话框代码
	tipsHtml : function(str) {
		if($(".ajaxtips").length == 0) gfAjax.loading();
		$(".ajaxtips").html(str);
	...
	debug : function(data) {
		...
		gfAjax.tipsHtml('<div class="ajaxbox bfalse">'+ msg +'<u>\u6211\u77E5\u9053\u4E86</u></div>');
		gfAjax.setTopAn();

		$(".ajaxtips u").click(gfAjax.close);
	},

	//提示框
	alert : function(data) {
		window.gfData = data = toJson(data);
		if(window.gfExit) return;

		gfAjax.tipsHtml('<div class="ajaxbox b'+ (data.err==0 ? true : false) +'"><i></i><b>'+ data.msg +'</b><u>\u6211\u77E5\u9053\u4E86</u></div>');
		gfAjax.setTopAn();

		$(".ajaxtips u").click(function(){
			gfAjax.close();
			if(!window.gfName && data.name != '') $("[name='"+data.name+"']").focus();
		});
		if(!window.gfErr && data.err==0) setTimeout(gfAjax.close, 1000);
	},

	//确定框
	confirm : function(msg, func) {
		gfAjax.tipsHtml('<div class="ajaxbox bnote"><i></i><b>'+ msg +'</b></div>');
		$(".ajaxbox").append('<p class="cf"><a id="noA" class="but3">取消</a><a id="okA" class="but3">确认</a></p>');
		gfAjax.setTopAn();

		$("#noA,#okA").attr("href","javascript:;");
		$("#noA").click(gfAjax.close);
		$("#okA").click(function(){ gfAjax.remove(); func(); });
	...
	submit : function(selector, callback) {
		$(selector).submit(function(){
			gfAjax.postd($(this).attr("action"), $(this).serialize(), callback);
			return false;
		});
	},
	...
	postd : function(url, param, callback) {
		gfAjax.loading();
		gfAjax.post(url, param, (!callback ? gfAjax.alert : callback));
	},
...
$.gfDialog = function(options) {
	...
	$("#twdialog_title a,#twdialog_button .close").click(function(){
		if(o.remove) { $.gfDialog("remove"); }else{ $.gfDialog("close"); }
	});
};
