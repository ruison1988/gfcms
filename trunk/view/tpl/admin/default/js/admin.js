...
$(function() {
	...
	$("#closeer,#adder,#leftbtn,#rightbtn").hover(function() {
		$(this).addClass("on");
	}, function() {
	...
	$("#ifr_refresh").click(function() {
		ifrRefresh($("#box_tab ul li.on").index());
		return false;
	});
...
function oneTab(urlKey) {
	...
	if(len > 0) {
		...
		$("#box_frame iframe:eq(" + newTab.index() + ")").attr("src", urlKey);
	} else {
		addTab("正在加载中", urlKey);
	}
}
...
function addTab(title, urlKey) {
	...
	$("#box_frame").append('<iframe src="' + urlKey + getR() + '" frameborder="0" scrolling="yes"></iframe>');
...
//加载事件
function loadEvent() {
	$("#box_tab ul li:last").click(function() {
		onTab($(this))
	});
	$("#box_tab ul li i:last,#box_tab ul li b:last").hover(function() {
		$(this).addClass("on")
	}, function() {
		$(this).removeAttr("class")
	});
...
function onTab(obj) {
	...
	if(valLeft != "no") $("#box_tab ul").animate({
		left: valLeft
	}, "fast").css("left", valLeft);
	setAdder();
...
function rightTab() {
	...
	for(var i = 0; i < $("#box_tab ul li").length; i++) {
		if($("#box_tab ul li").eq(i).offset().left >= 167 + widthMax - $("#box_tab ul li").eq(i).width()) {
			...
			$("#box_tab ul").animate({
				left: vLeft <= widthMax - ulWidth ? widthMax - ulWidth : vLeft
			}, "fast");
			break;
		}
	}
