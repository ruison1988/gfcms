...
<div class="m">
	<div class="head">
		<dl>
			...
			<dt style="display:inline;float:right">
				<form class="search" method="post" action="http://<?php echo(isset($C['hostname']) ? $C['hostname'] : ''); ?>/index.php?u=photo-index">
				<input type="submit" value="搜索图集" class="but1" style="float:right;margin:0 8px 0 5px">
	...
	<div class="p c2">
		<div class="cc">
			<?php if (empty($cms_photo_arr)) { ?><div class="sky warning bnote"><i></i><b>暂无内容</b></div><?php }else{ ?>
			<table class="tb2">
				<tr class="tit">
				...
				<?php if(isset($cms_photo_arr) && is_array($cms_photo_arr)) { foreach($cms_photo_arr as &$v) { ?>
				<tr class="li">
...
<script type="text/javascript">
$("#add").click(function(){
	parent.oneTab("photo-add");
});

$("#cid").change(function(){
	location.href = "index.php?u=photo-index-cid-"+ $(this).val();
});
...
function edit(id, cid) {
	parent.oneTab("photo-edit-cid-"+cid+"-id-"+id);
}

//删除
function del(id, cid) {
	gfAjax.confirm("删除不可恢复，确定删除？", function(){
		gfAjax.postd("index.php?u=photo-del-ajax-1", {"id":id, "cid":cid}, function(data){
			gfAjax.alert(data);
...
function batch_del() {
	gfAjax.confirm("删除不可恢复，确定删除？", function(){
		...
		gfAjax.postd("index.php?u=photo-batch_del-ajax-1", {"id_arr":id_arr}, function(data){
			gfAjax.alert(data);
			if(window.gfData.err==0) setTimeout(function(){ window.location.reload(); }, 1000);
		});
	});
