...
<div class="m">
	<div class="head">
		<dl>
			...
			<dt style="display:inline;float:right">
				<form class="search" method="post" action="http://<?php echo(isset($C['hostname']) ? $C['hostname'] : ''); ?>/index.php?u=product-index">
				<input type="submit" value="搜索产品" class="but1" style="float:right;margin:0 8px 0 5px">
	...
	<div class="p c2">
		<div class="cc">
			<?php if (empty($cms_product_arr)) { ?><div class="sky warning bnote"><i></i><b>暂无内容</b></div><?php }else{ ?>
			<table class="tb2">
				<tr class="tit">
				...
				<?php if(isset($cms_product_arr) && is_array($cms_product_arr)) { foreach($cms_product_arr as &$v) { ?>
				<tr class="li">
...
<script type="text/javascript">
$("#add").click(function(){
	parent.oneTab("product-add");
});

$("#cid").change(function(){
	location.href = "index.php?u=product-index-cid-"+ $(this).val();
});
...
function edit(id, cid) {
	parent.oneTab("product-edit-cid-"+cid+"-id-"+id);
}

//删除
function del(id, cid) {
	gfAjax.confirm("删除不可恢复，确定删除？", function(){
		gfAjax.postd("index.php?u=product-del-ajax-1", {"id":id, "cid":cid}, function(data){
			gfAjax.alert(data);
...
function batch_del() {
	gfAjax.confirm("删除不可恢复，确定删除？", function(){
		...
		gfAjax.postd("index.php?u=product-batch_del-ajax-1", {"id_arr":id_arr}, function(data){
			gfAjax.alert(data);
			if(window.gfData.err==0) setTimeout(function(){ window.location.reload(); }, 1000);
		});
	});
