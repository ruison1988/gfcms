...
include RUNTIME_CONTROL.'admin_control.class.php'; class plugin_control extends admin_control {
	// 插件管理
	...
	public function install() {
		...
		$cms_version && version_compare($cms_version, C('version'), '>') && E(1, '无法安装，最低版本要求：GFCMS '.$cms_version);
	...
	public function install_plugin() {
		...
		// ======  开始安装 ======
		$cms_version = $this->get_version($dir);
		$cms_version && version_compare($cms_version, C('version'), '>') && $this->install_tips('无法安装，最低版本要求：GFCMS '.$cms_version);
	...
	private function download($dir, $is_upgrade = FALSE) {
		...
		$url = 'http://www.gfcms.com/app/download.php?plugin='.$dir.($is_upgrade ? '&upgrade=1' : '');
		try{
