...
include RUNTIME_CONTROL.'admin_control.class.php'; class setting_control extends admin_control {
	// 基本设置
	...
	public function link() {
		if(empty($_POST)) {
			...
			$parseurl = $_ENV['_config']['parseurl'];
			$cfg = $this->kv->xget('cfg');
			$this->assign('cfg', $cfg);
			$mk = R('mk');
			$del = R('del');
			...
			$iis .= "\t\t".'<rewrite>'."\r\n";
			$iis .= "\t\t\t".'<rules>'."\r\n";
			$iis .= "\t\t\t\t".'<rule name="GFCMS Rule '.$cfg['webdir'].'" stopProcessing="true">'."\r\n";
			...
			if(!_is_writable($file)) exit('{"err":1, "msg":"配置文件 '.CONFIG_PATH.'config.inc.php 不可写"}');
			$s = file_get_contents($file);
			$s = preg_replace("#'parseurl'\s*=>\s*\d,#", "'parseurl' => {$parseurl},", $s);
